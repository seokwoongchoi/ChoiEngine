...
{
    uint instId = lerp(InstID, InstID + Tweenframes[actorIndex - 1].drawCount, actorIndex);
                        ...
                        uint3 groupThreadId : SV_GroupThreadID)
{
    ...
    if (groupThreadId.x < Tweenframes[groupId.y].maxBoneCount)
    {
        for (int i = 0; i < Tweenframes[groupId.y].drawCount; i++)
        {
            ...
            uint frameIndex = lerp(i, i + Tweenframes[groupId.y - 1].drawCount, groupId.y);
           ...
           [unroll(3)]
            for (uint b = 0; b < 3; b++)
            {
                int boneIndex = boneBoxTransforms[int2((b * 9 + 8), groupId.y)].x;
              
                if (groupThreadId.x == boneIndex)
                {
                    Matrix local;
                    GetBoneBoxtMatrix(local, groupId.y, b, 0);
                    ...
                    Matrix BoneScale;
                    GetBoneBoxtMatrix(BoneScale, groupId.y, b, 1);
                    matrix compute = mul(BoneScale, compose(position, q, float3(1, 1, 1)));
                    
                    [flatten]
                    if (b == 2 && Tweenframes[frameIndex].Curr.Clip == 3)
                    {
                        [flatten]
                        if (Tweenframes[frameIndex].Curr.CurrFrame < 17 || Tweenframes[frameIndex].Curr.CurrFrame > 22)
                            compute = 0;

                    }
        
                  //  GroupMemoryBarrierWithGroupSync();
                    Output[uint2(b * 4 + 0, frameIndex)] = float4(compute._11, compute._12, compute._13, compute._14);
                    Output[uint2(b * 4 + 1, frameIndex)] = float4(compute._21, compute._22, compute._23, compute._24);
                    Output[uint2(b * 4 + 2, frameIndex)] = float4(compute._31, compute._32, compute._33, compute._34);
                    Output[uint2(b * 4 + 3, frameIndex)] = float4(compute._41, compute._42, compute._43, compute._44);
                }
