...
int Collison(matrix Box1,matrix Box2)
{
...
[numthreads(10, 1, 1)]
void CS(uint3 globalThreadId : SV_DispatchThreadID)//uint3 GroupID:SV_GroupID,uint GroupIndex:SV_GroupIndex
{
    int result = 0;
  
    matrix Box1 = CreateBox(globalThreadId.x,0);
    matrix Box2 = CreateBox(globalThreadId.y, 0);
    
   
     //[flatten]
    if (globalThreadId.x!= globalThreadId.y)
    {
       
        //result = IntersectionSphere(Box1, Box2);
       result = Collison(Box1, Box2);
    }
  

   
    float4 BodyMin = mul(BoundsMin, Box1);
    float4 BodyMax = mul(BoundsMax, Box1);
    int Bodyintersection = IntersectionAABB(Position, Direction, BodyMin.xyz, BodyMax.xyz);
    
    matrix HeadMatrix = CreateBox(globalThreadId.x, 1);
    float4 HeadMin = mul(BoundsMin, HeadMatrix);
    float4 HeadMax = mul(BoundsMax, HeadMatrix);
    int HeadIntersection = IntersectionAABB(Position, Direction, HeadMin.xyz, HeadMax.xyz);
   ...
   // CopyOutput[uint2(1, globalThreadId.x)] = float4(Input[uint2(8, globalThreadId.x)].r, Input[uint2(8, globalThreadId.x)].g, result | Bodyintersection, HeadIntersection);
    CopyOutput[globalThreadId.x].Indices.x = Input[uint2(8, globalThreadId.x)].r;//actor Index
    CopyOutput[globalThreadId.x].Indices.y = Input[uint2(8, globalThreadId.x)].g;//Instance Num
    CopyOutput[globalThreadId.x].Indices.z = Input[uint2(8, globalThreadId.x)].b;//culled drawCount
    CopyOutput[globalThreadId.x].Indices.w = Input[uint2(8, globalThreadId.x)].a;//ActorType 0=skin, 1=static
    
    CopyOutput[globalThreadId.x].Factor.x = Bodyintersection;
    CopyOutput[globalThreadId.x].Factor.y = HeadIntersection;
    CopyOutput[globalThreadId.x].Factor.z = result ;
    CopyOutput[globalThreadId.x].Factor.w = 0;
    ...
    if (result == 1)
    {
        direction=float4(Box2._41, Box2._42, Box2._43, 0) - float4(Box1._41, Box1._42, Box1._43, 0);

    }
   
    CopyOutput[globalThreadId.x].Direction = direction;
