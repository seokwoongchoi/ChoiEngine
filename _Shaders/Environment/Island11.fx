...
Texture2D LayerdefTexture;
Texture2D RockBumpTexture;
Texture2D RockMicroBumpTexture;
Texture2D RockDiffuseTexture;
Texture2D SandBumpTexture;
Texture2D SandMicroBumpTexture;
Texture2D SandDiffuseTexture;
Texture2D GrassDiffuseTexture;
Texture2D SlopeDiffuseTexture;
Texture2D WaterBumpTexture;
...
Texture2D RefractionDepthTextureResolved : register(t2);
Texture2D WaterNormalMapTexture : register(t3);


Texture2DMS<float,1> RefractionDepthTextureMS1;
...
{
    return  DynamicTessFactor * (1 / (0.06f * distance));
}
...
float4 CombineWaterNormal(float3 world_position)
{
    ...
    for (float i = 0; i < 5; i++)
    {
        texvalue = WaterBumpTexture.SampleLevel(SamplerLinearWrap, tc * texcoord_scale + WaterBumpTexcoordShift * 0.03 * variance, water_miplevel).rbga;
        variance.x *= -1.0;
...
float3 CombineSimplifiedWaterNormal(float3 world_position, float mip_level)
{
    ...
    for (float i = 0; i < 8; i++)
    {
        texvalue = WaterBumpTexture.SampleLevel(SamplerLinearWrap, tc * texcoord_scale + WaterBumpTexcoordShift * 0.03 * variance, mip_level /*+i*/).rbga;
        variance.x *= -1.0;
...
float CalculateWaterCausticIntensity(float3 worldpos)
{
    ...
    float distance_to_camera = length(CameraPosition - worldpos);
    ...
    for (float i = -3; i <= 3; i += 1)
        ...
        {
            n = 2.0f * WaterNormalMapTexture.SampleLevel(SamplerLinearWrap, (worldpos.xz - CameraPosition.xz - float2(200.0, 200.0) + float2(i * k, j * k) * m * water_depth) / 400.0, 0).rgb - float3(1.0f, 1.0f, 1.0f);
...
PatchData PatchConstantHS(InputPatch<HSIn_Heightfield, 1> inputPatch)
{
    ...
    float3 patch_center = float3(inputPatch[0].origin.x + inputPatch[0].size.x * 0.5, TerrainBeingRendered * HeightfieldTexture.SampleLevel(SamplerLinearWrap, texcoord0to1, 0).w, inputPatch[0].origin.y + inputPatch[0].size.y * 0.5);
    float3 camera_to_patch_vector = patch_center - CameraPosition;
                                    ...
                                    OutputPatch<DUMMY, 1> inputPatch)
{
    ...
    base_texvalue = HeightfieldTexture.SampleLevel(SamplerLinearWrap, texcoord0to1, 0);
    ...
    distance_to_camera = length(CameraPosition - vertexPosition);
    ...
    layerdef = LayerdefTexture.SampleLevel(SamplerLinearWrap, texcoord0to1, 0);
    ...
    output.position = mul(float4(vertexPosition, 1.0), ModelViewProjectionMatrix);
   ...
   // output.depthmap_scaler = float4(1.0, 1.0, , cc);
...
float4 HeightFieldPatchPS(PSIn_Diffuse input) : SV_Target
{
	...
	// culling halfspace if needed
     clip(HalfSpaceCullSign * (input.positionWS.y - HalfSpaceCullPosition));
    ...
    color.rgb *= max(0, dot(pixel_to_light_vector, microbump_normal))  + 0.2;
...
[domain("quad")]
PSIn_DiffuseWater WaterPatchDS(PatchData input,
                                    float2 uv : SV_DomainLocation,
                                    OutputPatch<DUMMY, 1> inputPatch)
{
    PSIn_DiffuseWater output;
    float3 vertexPosition;
    float2 texcoord0to1 = (input.origin + uv * input.size) / HeightFieldSize;
    float4 water_normal;
    ...
    depthmap_scaler = DepthMapTexture.GatherGreen(SamplerLinearWrap, float2(texcoord0to1.x, 1 - texcoord0to1.y), 0);
	...
	// writing output params
    output.position = mul(float4(vertexPosition, 1.0), ModelViewProjectionMatrix);
    output.texcoord = texcoord0to1 * WaterMicroBumpTexcoordScale + WaterBumpTexcoordShift * 0.07;
    output.normal = normalize(water_normal.xyz);
    output.positionWS = float4(vertexPosition.xyz, depthmap_scaler);
    return output;
}

PS_GBUFFER_OUT WaterPatchPS(PSIn_DiffuseWater input) 
{
    float4 color;
    float3 pixel_to_light_vector = normalize(LightPosition - input.positionWS.xyz);
    float3 pixel_to_eye_vector = normalize(CameraPosition - input.positionWS.xyz);
    ...
    microbump_normal = normalize(2 * WaterBumpTexture.Sample(SamplerAnisotropicWrap, input.texcoord * 0.5 + WaterBumpTexcoordShift * 0.05).gbr - float3(1, -8, 1));
    microbump_normal += normalize(2 * WaterBumpTexture.Sample(SamplerAnisotropicWrap, input.texcoord * 0.5 + WaterBumpTexcoordShift * 0.05).gbr - float3(1, -8, 1));
  ...
  // simulating scattering/double refraction: light hits the side of wave, travels some distance in water, and leaves wave on the other side
    ...
    scatter_factor +=  1.5 * WaterColorIntensity.y * max(0, input.positionWS.y + 1) *
		...
		max(0, 1 - pixel_to_eye_vector.y) * (300.0 / (300 + length(CameraPosition - input.positionWS.xyz)));
    ...
    disturbance_eyespace = mul(float4(microbump_normal.x, 0, microbump_normal.z, 0), ModelViewMatrix);
    ...
    float2 refraction_disturbance = float2(-disturbance_eyespace.x, disturbance_eyespace.y) * 0.05 *
		...
		(20.0 / (20 + length(CameraPosition - input.positionWS.xyz)));
	
	// calculating correction that shifts reflection up/down according to water wave Y position
    float4 projected_waveheight = mul(float4(input.positionWS.x, input.positionWS.y, input.positionWS.z, 1), ModelViewProjectionMatrix);
    float waveheight_correction = -0.5 * projected_waveheight.y / projected_waveheight.w;
    projected_waveheight = mul(float4(input.positionWS.x, -0.8, input.positionWS.z, 1), ModelViewProjectionMatrix);
    ...
    float4 vertex_in_viewspace = mul(float4(input.positionWS.xyz, 1), ModelViewMatrix);
...
{
    float4 color;
    color.rgb = (CombineSimplifiedWaterNormal(CameraPosition + float3(input.texcoord.x * 400.0f - 200.0f, 0, input.texcoord.y * 400.0f - 200.0f), 0).rgb + float3(1.0f, 1.0f, 1.0f)) * 0.5f;
    color.a = 0;
    return color;
}
