...
cbuffer CB_View : register(b1)
{
    ...
    float4 LightDirVS : packoffset(c4);
...
//PS_GBUFFER_OUT RenderSceneParallaxMappingPS(VertexModelParallaxMappingOutput In)
//{
...
//        float2 vTexOffsetPerStep = fStepSize * In.ParallaxOffset;
...
//            fCurrHeight = NormalMap.SampleGrad(LinearSampler, vTexCurrentOffset, dx, dy).b;
...
//        float2 vParallaxOffset = In.ParallaxOffset * (1 - fParallaxAmount);
...
PS_GBUFFER_OUT RenderSceneParallaxMappingPS(VertexModelParallaxMappingOutput In)
{
    ...
    float fCurrentHeight = normalize(NormalMap.Sample(LinearSampler, In.Uv).b);
    
    float fHeight = fCurrentHeight * 0.1f + 0.01f;
