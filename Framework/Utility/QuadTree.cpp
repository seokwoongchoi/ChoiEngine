...
void QuadTree::Intersection( Matrix *const matrix, uint& ID)
{
	...
	Vector2 lerp;
	D3DXVec2Lerp(&lerp, &Vector2(0, matrix->m[3][1]), &Vector2(0,actorPos.y), Time::Delta()*5.0f);
	matrix->m[3][1] = lerp.y;
	ID = id;
}
...
void QuadTree::GetRay(Ray* ray,const Matrix & v, const Matrix & p)
{
	...
	//inv View
	{
		...
		D3DXVec3TransformNormal(&ray->dir, &Vector3(point.x, point.y, 1), &invView);
	...
	//inv world
	{
		...
		D3DXVec3TransformCoord(&ray->org, &cameraPosition, &invWorld); //직선이 맞을 물체와 카메라의 공간을 일치시켜주기위해서 
		D3DXVec3TransformNormal(&ray->dir, &ray->dir, &invWorld);//직선이 맞을 물체와 카메라의 공간을 일치시켜주기위해서
		D3DXVec3Normalize(&ray->dir, &ray->dir);
		D3DXVec3TransformCoord(&ray->org, &ray->org, &invWorld);
	}
}
...
bool QuadTreeNode::Intersection(const Ray& ray, Vector3 & Pos, uint& ID, const uint& FindHierarchyIndex)
{
	...
	for (auto& p : hittedChilds)
	{
		...
		float l1 = D3DXVec3LengthSq(&(ray.org - thisHit));
		float l2 = D3DXVec3LengthSq(&(ray.org - bestHit));
...
bool QuadTreeNode::IntersectionAABB(const Ray & ray, Vector3 & Pos, float & d)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (abs(ray.dir[i]) < Math::EPSILON)
		{
			if (ray.org[i] < boundsMin[i] ||
				ray.org[i] >boundsMax[i])
			{
				return false;
			}
		...
		{
			float denom = 1.0f / ray.dir[i];
			float t1 = (boundsMin[i] - ray.org[i]) * denom;
			float t2 = (boundsMax[i] - ray.org[i]) * denom;
