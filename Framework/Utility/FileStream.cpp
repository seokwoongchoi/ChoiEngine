...
void FileStream::Write(const std::vector<byte>& value)
{
    auto length = static_cast<uint>(value.size());
    Write(length);

    out.write(reinterpret_cast<const char*>(value.data()), sizeof(byte) * length);
}
...
void FileStream::Read(std::vector<byte>& value)
{
    auto length = Read<uint>();
    ...
    in.read(reinterpret_cast<char*>(value.data()), sizeof(byte) * length);
}