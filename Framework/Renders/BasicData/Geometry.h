...
public:
    ...
    const uint GetVertexDataSize() const { return static_cast<uint>(vertices.size()) * sizeof(T); }
    const uint GetIndexDataSize() const { return static_cast<uint>(indices.size()) * sizeof(uint); }

	inline const std::vector<T>& GetVertices() const { return vertices; }
	void GetVertices
	(
		const uint& offset,
		const uint& count,
