	...
	outputTexture(nullptr),outputSRV(nullptr),outputUAV(nullptr)
{
	...
	boneTransfomrs = new SkinnedTransform[2* MAX_CLIP_SIZE];
	...
	tweenDesc.assign(MAX_MODEL_INSTANCE, TweenDesc());
	tweenData.assign(MAX_MODEL_INSTANCE, TweenData());
	
	colliderBoxData.assign(6, BoneBoxDesc());
...
uint Animator::Update(const uint& actorIndex)
{
	...
	if (actorIndex > 0)
	{
		start+= renderDatas[actorIndex-1].drawCount;
	}
	...
	for (uint index = start; index < end; index++)
	{
		...
		if (tween.state == ActorState::Die)
		{
			...
			if (tween.Curr.NextFrame >= clips[static_cast<uint>(tween.state)]->Duration() - 1)
			{
				tween.Curr.CurrFrame = static_cast<uint>(clips[static_cast<uint>(tween.state)]->Duration()) - 2;
				tween.Curr.NextFrame = static_cast<uint>(clips[static_cast<uint>(tween.state)]->Duration()) - 1;
		    	continue;
			}
		}
		else
		...
		if (tween.Next.Clip > -1)
		{
			...
			{
				if (tween.bContinue == true)
				{
					
					int temp = tween.Curr.CurrFrame - tween.farmeDelta;
					tween.Next.CurrFrame = temp;
					tween.bContinue = false;
				}
				tween.RunningTime += Time::Delta();
	...
	for (uint i = 0; i < renderDatas[actorIndex].drawCount; i++)
	{
		tree->Intersection(&instTransforms[actorIndex][i]);
		if (actorIndex < 1 && i < 1)continue;
		transforms->Update(actorIndex, i);
	}
		...
		uint boneCount = 0; 
			...
			if (!clips.empty())
			{
				for (uint i = 0; i < clipSize; i++)
				{
					...
					boneCount = boneTransfomrs[0].bones[boneActorIndex].size();
				...
				for (uint i = 0; i < clipSize; i++)
				{
					...
					tweenData[boneActorIndex].maxBoneCount = boneTransfomrs[0].bones[boneActorIndex].size();
				}
