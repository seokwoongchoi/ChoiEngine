	...
	, AdditiveBlendState(nullptr), depthStencilState(nullptr)
{
	...
	cloudTexture1->Load(device,L"Environment/cloud001.dds");
	cloudTexture2 = new Texture();
	cloudTexture2->Load(device,L"Environment/cloud002.dds");
...
{
	ID3D11ShaderResourceView* srvArray[3] = { srv,*cloudTexture1,*cloudTexture2 };
...

	{
		...
		D3DXMatrixTranspose(&vsDesc.wvp, &(S*T * GlobalData::GetVP()));
...
void Cloud::Render(ID3D11DeviceContext * context, const Matrix & VP)
{
...

	{
		...
		Vector3 camPos = GlobalData::Position();
		Matrix S, T;
		Matrix ViewProj = VP * GlobalData::GetProj();
		D3DXMatrixTranslation(&T, camPos.x, camPos.y, camPos.z);
		D3DXMatrixScaling(&S, scale, scale, scale);
