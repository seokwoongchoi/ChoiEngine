    ...
    capsuleCount(0)
{
	...
	pointTexture->Load(device, L"LightIcons/PointLight.png");
	spotTexture = new Texture();//
	spotTexture->Load(device, L"LightIcons/SpotLight.png");
	capsuleTexture = new Texture();//
	capsuleTexture->Load(device, L"LightIcons/CapsuleLight.png");
...
{
	ImGui::Begin("Lights", 0, ImGuiWindowFlags_NoMove);
...
void LightEditor::PointLightRender()
{
	if (pointCount > 0)
	{
		const Matrix& V = GlobalData::GetView();
		const Matrix& P = GlobalData::GetProj();
		auto pointLight = Lighting.GetPointLights();
		for (uint i = 0; i < pointCount; i++)
		{
...
void LightEditor::PointLightControll(int index)
{
	...
	if (ImGui::CollapsingHeader("PointLight"))
	{
		...
		ImGui::SliderFloat("LightPositionX", &pointLight[index].Position.x, 0.0f, 512.0f);
		ImGui::SliderFloat("LightPositionY", &pointLight[index].Position.y,-10.0f,50.0f);
		ImGui::SliderFloat("LightPositionZ", &pointLight[index].Position.z,0.0f,512.0f);
