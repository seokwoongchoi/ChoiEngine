...
void SetModelWorld(inout matrix world, VertexModel input)
{
    ...
    for (int i = 0; i < 4;i++)
    {
        float4 m1 = BoneTransforms.Load(int4(boneIndices[i] * 4 + 0, input.InstID, 0, 0));
        float4 m2 = BoneTransforms.Load(int4(boneIndices[i] * 4 + 1, input.InstID, 0, 0));
        float4 m3 = BoneTransforms.Load(int4(boneIndices[i] * 4 + 2, input.InstID, 0, 0));
        float4 m4 = BoneTransforms.Load(int4(boneIndices[i] * 4 + 3, input.InstID, 0, 0));

        temp = matrix(m1, m2, m3, m4); //각행별로 들어간다.
        transform += mul(boneWeights[i], temp);
    }
      
  
    world = mul(transform, input.Transform);
}
void SetStaticModelWorld(inout matrix world, VertexModel input)
{
  //  world = mul(BoneTransforms[BoneIndex], world); //단일 모델용 함수따로만들어야함
    
    //float4 m1 = BoneTransforms.Load(int3(BoneIndex * 4 + 0, input.InstID, 0)); //마지막은 mipmap
    //float4 m2 = BoneTransforms.Load(int3(BoneIndex * 4 + 1, input.InstID, 0));
    //float4 m3 = BoneTransforms.Load(int3(BoneIndex * 4 + 2, input.InstID, 0));
    //float4 m4 = BoneTransforms.Load(int3(BoneIndex * 4 + 3, input.InstID, 0));
    ...
    matrix transform = matrix(m1, m2, m3, m4); //각행별로 들어간다.
...
MeshOutput VS_Model(VertexModel input)
{
    ...
    SetStaticModelWorld(World, input);
...
MeshOutput VS_StaticModel(VertexModel input)
{
    MeshOutput output;
    ...
    output.Clip = 0;
    //output.Clip = dot(float4(1, 0, 0, 0), WorldPosition(input.Position));
...
void SetAnimationWorld(inout matrix world, VertexModel input)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (clip[1] >= 0)
        {
            ...
            anim2 = lerp(anim, nextAnim, Tweenframes[input.InstID].TweenTime);
        }

        transform += mul(boneWeights[i], anim2);
...
MeshOutput VS_ModelAnimation(VertexModel input)
{
    MeshOutput output;
...
MeshOutput VS_ModelAnimation_GS(VertexModel input)
{
    MeshOutput output;
